...
d = pd.read_csv(f"{BASE}/results/uscrn_tobs_counts3.csv")
...
cols = [f"h{H}_b{k}" for H in range(24) for k in range(len(BIASES))]
...
out = {
    ...
    # matrix[y][H][k]
    "matrix": [[[round(float(mat.loc[y, f"h{H}_b{k}"]), 3) for k in range(len(BIASES))]
...
out["episode_station_2012"] = {"true": int(r.h0_b0), "pm5": int(r.h17_b0),
                               "am7": int(r.h7_b0), "pm5_mmts": int(r.h17_b4)}
...
pm5_m = np.array([mat.loc[y, "h17_b4"] for y in YEARS])   # 5PM + MMTS -0.4C
am7_m = np.array([mat.loc[y, "h7_b4"] for y in YEARS])    # 7AM + MMTS -0.4C

scen = {
    "true": true,
    "tobs only (5PM->7AM)": (1 - f) * pm5 + f * am7,
    "mmts only -0.4C (at 5PM)": (1 - f) * pm5 + f * pm5_m,
    "combined (5PM->7AM, -0.4C)": (1 - f) * pm5 + f * am7_m,
}
print(f"\nMMTS -0.4C effect on counts (5PM obs): {pm5_m.mean()/pm5.mean():.3f}x")
